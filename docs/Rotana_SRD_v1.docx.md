...
| **Backend** | Node.js, Express.js with modular REST API architecture |
| **Database** | PostgreSQL via Supabase (relational, row-level security for RBAC) |
| **Auth** | JWT-based authentication with role guards and refresh token rotation |
...
| **State Mgmt** | Zustand (lightweight global state for dashboards) |
| **Hosting** | Vercel (frontend) \+ Supabase (backend/DB) or VPS with Nginx \+ PM2 |
| **CI/CD** | GitHub Actions for automated deployments |
| **Security** | Helmet, CORS, rate limiting, input validation (Zod), SQL injection prevention |
